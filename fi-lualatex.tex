...
\thesissetup{
    ...
    keywords    = {React Native, OCR, mobile development, Android, Windows, testing, continuous integration, natural language processing, word embedding, machine learning, sales receipts, cross-platform development, image processing, OpenCV},
    % TeXkeywords = {keyword1, keyword2, \ldots},
    abstract    = {%
    
    This thesis aims to develop a mobile and desktop application to store sales receipts in a digitalised form, removing the need to keep the original. The application uses optical character recognition to extract data from a receipt image and pre-fill the database record. It utilises word embeddings to assign emojis to receipt items and image processing techniques to transform the receipt photo into a more readable form.
      
    The theoretical part introduces React Native in the context of other cross-platform development frameworks.
    
    An emphasis is put on testing and continuous integration as an integral part of creating reliable software.
    
    % Cílem této práce je vytvořit mobilní a desktopovou aplikaci pro ukládání účtenek v digitální formě, nahrazující potřebu pro skladování originálů účtenek.
    % Aplikace používá optické rozpoznávání znaků pro extrakci dat z fotky účtenky a předvyplnění databázového záznamu. 
    % Využitím vnoření slov (word embeddings) je aplikace schopna přiřadit emodži jednotlivým položkám účtenky. Pomocí technik pro zpracování obrazu je fotka účtenky převedena do čitelnější podoby.
    
    % Teoretická část práce představuje React Native v kontextu aplikačních rámců pro vývoj multiplatformních aplikací.
    
    % Důraz je kladen na testování a průběžnou integraci jako nedílnou součást vývoje spolehlivého softwaru.
    ...
    thanks      = {%
      ...
      I would especially like to thank my sister for helping me proofread the thesis.
      
      I wish to express my deepest gratitude to my girlfriend for her endless support, encouragement and advice.
    },
    bib         = bibliography.bib,
    %% Uncomment the following line (by removing the %% at the
    %% beginning) and replace `assignment.pdf` with the filename
    %% of your scanned thesis assignment.
    %% Applicable for 'printed' thesis only.
...
\newcommand{\code}[1]{\mintinline[]{javascript}|#1|} % TODO change to jsx and compile locally using 'latexmk -lualatex --shell-escape fi-lualatex.tex' for jsx syntax support (https://tex.stackexchange.com/a/519333)
...
\newcommand\half{0.45}
% subfigsize is relative to the subfigure
...
In the first chapter, I describe currently available applications trying to solve this task, their main features and drawbacks. 

In the second chapter, I provide an overview of techniques used for data extraction from images. 

The next chapter focuses on choosing target platforms for which the application should be developed. 

The fourth chapter introduces available technologies for developing cross-platform applications. It also provides an overview of the chosen framework \Dash React Native \Dash which is further elaborated in the subsequent chapter.

An essential part of the thesis is testing and continuous integration. The types of testing used to ensure the high quality and reliability of the application are described in the sixth chapter. 

The seventh chapter describes the implementation and functionality of the application. It shows how Microsoft Form Recognizer~\cite{WhatIsFormRecognizer} was used to extract the information from a receipt photo. This chapter also includes a description of a receipt item categorization 1 using word embeddings. The image processing using the OpenCV\footnote{\url{https://opencv.org/}} library is featured as well. 
...
Table~\ref{table:models_comparison} shows results from various word embeddings models. The first two columns are produced by Google's word2vec, which was trained on 100 billion words from Google News~\cite{Google2013Word2Vec}. The third column is fastText. The fourth column is fastText with subwords information. In the subwords model, each word is represented as a bag of character $n$-grams. Taking the
...
\begin{enumerate}
    ...
    \begin{figure}
        ...
        \end{subfigure}
        \caption{Comparison of an image before and after Gaussian blur.}
    ...
    The result of applying dilation is shown in Figure~\ref{fig:dilation}.
    ...
    \begin{figure}[H]
        ...
        \end{subfigure}
        \caption{Comparison of an image before and after dilation.}
        \label{fig:dilation}
    \end{figure}

    \item \textbf{Finding the corners of a receipt}
    
    As the next step, all external contours in the image are found. Contour is a curve joining points of the same color and intensity. The contour with the largest area is most probably the contour of a receipt. Four points that represent the corners of the receipt are then found in the contour and returned. The idea behind finding the corners is that the bottom-left corner has the smallest $x$ coordinate and the largest $y$ coordinate, the bottom-right corner has the largest $x$ a $y$ coordinates etc. The result of this step is visualized in Figure~\ref{fig:points_and_edges}.
    ...
    \begin{figure}
        ...
        \end{center}
        \caption{Photo of a receipt with detected corners.}
    ...
    \begin{figure}
        ...
        \begin{subfigure}[t]{\half\textwidth}
          ...
          \caption{Without Gaussian blur.}
        \end{subfigure}
        \begin{subfigure}[t]{\half\textwidth}
          \centering
          \includegraphics[width=\subfigsize\textwidth]{figures/image_processing/adaptive_thresholding}
          \caption{With Gaussian blur.}
        \end{subfigure}
        \caption{Result of adaptive thresholding applied on an image without and with Gaussian blur.}
